...
def Convolution_1D(
    ...
    kernel_len = len(array2)
    
    if padding == 'full':
        pad_w = kernel_len - 1
        array1_pad = np.pad(array1, pad_w, mode='constant')
    elif padding != 'valid':
        raise ValueError("padding must be 'full' or 'valid'")
    
    output_length = ((len(array1_pad) - kernel_len) // stride) + 1
    result = np.zeros(output_length)
    
    for i in range(output_length):
        start = i * stride
        end = start + kernel_len
        result[i] = np.sum(array1_pad[start:end] * array2)
